...
| AI Agenda | `/admin/agenda`, `/admin/agenda/rooms`, `/admin/agenda/tracks` | `/e/<slug>/schedule`, `/e/<slug>/schedule.ics` | Run **Auto-place remaining** to fill unscheduled sessions, then drag one into a clash and watch the conflict view name both the room and the speaker collision. (Deterministic planner, not a model — see README Limitations.) |
