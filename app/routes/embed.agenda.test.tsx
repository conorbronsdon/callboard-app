...
import EmbedAgenda, { loader } from "./embed.agenda";
...
function args(path = "", slug = EVENT_SLUG): LoaderArgs {
  return {
    request: new Request(`https://x.test/embed/${slug}/agenda${path}`),
    params: { slug },
    context: {},
...
function markup(data: LoaderData): string {
  return renderToStaticMarkup(
    EmbedAgenda({ loaderData: data, params: { slug: EVENT_SLUG } } as unknown as Parameters<
      typeof EmbedAgenda
    >[0]),
  );
}
...
describe("agenda embed", () => {
  ...
  it("MUST FIRE: anonymous SSR renders a day-grouped agenda", async () => {
    ...
    expect(html).toContain("data-embed-agenda-day");
  ...
  it("MUST FIRE: saved options beat all query parameters", async () => {
    ...
    const data = await loader(
      args(`?embed=${embed.id}&theme=light&track=Infrastructure&accent=%23ffffff&density=full`),
    );
  ...
  it("MUST NOT FIRE: disabled, unknown, and different-widget saved embeds 404", async () => {
    const disabled = await saveEmbed({ id: "disabled-agenda", enabled: false });
    await expect(loader(args(`?embed=${disabled.id}`))).rejects.toMatchObject({ status: 404 });
    const schedule = await saveEmbed({ id: "schedule-id", widget: "schedule" });
    await expect(loader(args(`?embed=${schedule.id}`))).rejects.toMatchObject({ status: 404 });
  ...
  it("MUST NOT FIRE: poisoned accent is absent and full density shows speakers", async () => {
